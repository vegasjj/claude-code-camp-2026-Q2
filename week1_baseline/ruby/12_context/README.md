...
The `~/.boukensharc` file supports key=value lines (one per line). Lines starting with `#` are comments.
...
```sh
BOUKENSHA_PATH=~/Sites/Claude-Code-Camp/week1_baseline/12_context
```

### Custom config directory

```sh
BOUKENSHA_DIR=~/Sites/Claude-Code-Camp/.boukensha
```

A bare path (no `KEY=`) is treated as `BOUKENSHA_PATH` for backwards compatibility:

```txt
~/Sites/Claude-Code-Camp/week1_baseline/12_context
```
