...
module Boukensha
  ...
  def self.run(
    ...
    api_key ||= case backend
                ...
                when :azure_foundry then ENV["AZURE_OPENAI_API_KEY"]
                when :gemini        then ENV["GEMINI_API_KEY"]
                when :ollama_cloud  then ENV["OLLAMA_API_KEY"]
                end
    azure_endpoint ||= ENV["AZURE_OPENAI_ENDPOINT"] if backend == :azure_foundry
    ...
    be = case backend
         ...
         when :azure_foundry then Backends::AzureFoundry.new(api_key: api_key, endpoint: azure_endpoint, model: model)
         ...
         else raise ArgumentError, "Unknown backend #{backend.inspect}. Use :anthropic, :openai, :azure_foundry, :gemini, :ollama, or :ollama_cloud."
         end
