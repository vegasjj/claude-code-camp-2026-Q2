...
| `backend:` | `:anthropic` | `:anthropic`, `:openai`, `:azure_foundry`, `:gemini`, `:ollama`, or `:ollama_cloud` |
| `api_key:` | `ENV["<PROVIDER>_API_KEY"]` | API key for the selected provider |
| `azure_endpoint:` | `ENV["AZURE_OPENAI_ENDPOINT"]` | Azure OpenAI resource endpoint (for `:azure_foundry`) |
